...
<p>Travel Earth application starts with a splash screen. The image above is the splash screen working when the user opens the Travel Earth app. The splash screen has an ImageView & transition animation. The transition animation is a custom animation XML file. In addition, I have added a plane sound to the splash screen with the use of MediaPlayer. So basically when the splash screen is running there is background music playing.</p>
...
<p>Once the splash screen finishes the app goes to the home page. The home page has a bottom navigation menu at the bottom (Home, Translator, Quiz, Key phrases, and settings) and it also consists of five buttons on the main screen. I have added images above of the home screen in light mode and dark mode. </p>
...
<p>At the bottom of the screen is the bottom navigation view. Users will be able to just press on the view they want to switch with just a click.</p>
...
<p>When the user clicks on the first button (Maps) the screen will load Google Maps which displays Top Tourists Attraction. As per the project requirements we only need to show 24 attractions (12 countries with 2 attractions each). I have used 66 attractions for my 12 country's to make it look better. The Top-rated tourist attraction data is fetched from a JSON file in the raw resource directory. Each data object is represented by a marker on a Google Map. I have added more tourists attractions to display the Cluster of the markers as per the image above. The marker's information window then displays the attraction's name, city name & coordinates (latitude & longitude). Also, I have implemented the extra feature of changing the map style. the current map view is Retro but if the user clicks on the three dots on the top-right screen the new display message opens (Change Map Style). If the user clicks on it then the new map style Aubergine loads on the screen. Users can choose to click on the change map style if they wish to go to the Retro Style.</p>
...
<p>When the user clicks on the Quiz button the screen will load an Interactive Quiz. Again, I am displaying the dark theme and light theme image. The recycler view displays the 12 countries which users can scroll up and down to choose the country they prefer for the quiz. Each country has its own quiz questions. In each country's details, I have added a country's name, map, and official language. Once users click on the country they want to play the quiz for the new screen will load with the first question. Quiz data is fetched from an API file from GitHub. Quiz questions include animals, culture, food, drink, geography & sport for the specific country. Each quiz has six questions. Five multi-choice &  one true or false. Multi-choice questions have four answers and each question has an image. I have also added a timer to each question. The user must answer each question within a 30 second time limit. If the answer is correct then a Toast message display the answer is correct and if it's wrong then a Toast message displays that the answer you clicked was wrong and also tells them the correct answer. Images provided above. If the user does not answer within thirty seconds time limit the next question loads and the user loses 1 from the score. Once the quiz is complete the new page will congratulate the user and display their total score for the quiz. On this page, users have a choice to click on the finish button which will route the user back to the home page or they click on the highest score which takes the user to the highest score page. The highest scores are stored in a Room Database table and it displays the highest score in a TextView.</p>
...
<p>When the user clicks on the Translator button the screen will load the translator page. Users need to write their text input in the top EditText. The user will need to choose a language from the spinner at the bottom left and press the translate button. The new translated result will then display in the TextView which is at the bottom of the screen. A custom ProgressDialog is displayed while the text is being translated. if the EditText is blank the app will show a Toast message saying please enter the text to translate. There is also text-to-speech support. Users click on the speech button and the translated text will be said in a voice with the help of the Text-To-Speech feature.</p>
...
<p>When the user clicks on the Key Phrases button the screen will load the Country phrases. The recycler view displays the 12 countries which users can scroll up and down to choose the country they prefer for the phrases. Each country has its own key phrases. In each country's details, I have added a country's name, map, and official language. Once users clicks on the country they want to view the phrases for the new screen will load the key phrases for that country.</p>
...
<p>When the user clicks on the Settings button the screen will load the settings page which has a toggle switch that controls the light mode and dark mode feature. On the settings page, the user will be able to switch between light mode and dark mode. When users chose the dark mode and then exit the application when they open the application again the same theme will be applicable until the user changes it back. I have provided four buttons under the toggle switch. The first button is Facebook. When the user clicks on the button it will load the Facebook webpage. The second button is GitHub. When the user clicks on the button it will load the GitHub webpage. The third button is for Travel Earth's Privacy Policy. When the user clicks on the privacy policy button it will load the webpage displaying the policy. It is hosted on the GitHub Project. The link to the policy is provided at the start of this documentation. The fourth button is for Travel Earth's Terms & Conditions. When the user clicks on the Terms & Conditions button it will load the webpage displaying the conditions. It is hosted on the Website policies webpage. The link to Terms & Conditions is provided at the start of this documentation.</p>
...
<p>The Travel Earth Application can be exited via a DialogFragment. If the user presses the back button once the Toast message asks the user if they want to exit the app then press twice. If the user presses the back button twice within a certain time then the exit DialogFragment. The DialogFragment prompts the user when the user double-taps the mobile device's back button. It asks the user if they are sure to exit the app. If users click NO it will take them back to the screen they were on but if they press YES then the Travel Earth's App will close. </p>
...
<p>Localization support is provided for each country. When user changes the language of there mobile then Travel Earth application also changes its language. The images above show first to change the language to Hindi (India's Official language) then the Travel Earth app also changes to Hindi. Then the language is changed to Portuguese (Brazil's Official Language) then the Travel Earth app also changes to Portuguese.</p>
